...
def _generate_cigar(seed):
    ...
    if rng.choice([True, False]):
        cigar = f"{rng.integers(1, 100)}S" + cigar
    return cigar


def _mutate_sequence(
    original, rng, max_subsitutions=50, max_insertions=50, max_deletions=50
):
    """
    ...
    n_subsitutions = rng.integers(max_subsitutions)
    subsitution_indices = rng.choice(
        np.arange(len(mutant)), size=n_subsitutions, replace=False
    )
    subsitution_values = rng.integers(len(original.alphabet), size=n_subsitutions)
    mutant.code[subsitution_indices] = subsitution_values

    # Random insertions
    n_insertions = rng.integers(max_insertions)
    insertion_indices = rng.choice(
        np.arange(len(mutant)), size=n_insertions, replace=False
    )
    insertion_values = rng.integers(len(original.alphabet), size=n_insertions)
    mutant.code = np.insert(mutant.code, insertion_indices, insertion_values)

    # Random deletions
    n_deletions = rng.integers(max_deletions)
    deletion_indices = rng.choice(
...
@pytest.mark.parametrize("distinguish_matches", [False, True])
@pytest.mark.parametrize("include_terminal_gaps", [False, True])
def test_alignment_conversion(seed, local, distinguish_matches, include_terminal_gaps):
    ...
    ref.code = rng.integers(0, len(ref.alphabet), REF_LENGTH, dtype=np.uint8)

    excerpt_start = rng.integers(0, 200)
    excerpt_stop = rng.integers(REF_LENGTH - 200, REF_LENGTH)
