...
def test_multiple_atom_array():
    ...
    test_file = gro.GROFile()
    test_file.set_structure(list(stack))

    # The header line of each model contains a timestamp, which may
    # differ between the two files -> ignore these lines in comparison
    ref_lines = [
        line for line in ref_file.lines if not line.startswith("Generated by Biotite")
    ]
    test_lines = [
        line for line in test_file.lines if not line.startswith("Generated by Biotite")
    ]
