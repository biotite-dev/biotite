...
class Sequence(Copyable, metaclass=abc.ABCMeta):
    ...
    size of the :class:`Alphabet`: If the :class:`Alphabet` contains 256 symbols or
    less, one byte is used per array element; if the :class:`Alphabet` contains
    between 257 and 65536 symbols, two bytes are used, and so on.
    
    Two :class:`Sequence` objects are equal if they are instances of the same
    class, have the same :class:`Alphabet` and have equal sequence codes.
    ...
    Each subclass of :class:`Sequence` needs to overwrite the abstract method
    `get_alphabet()`, which specifies the alphabet the :class:`Sequence` uses.
    ...
    ----------
    code : ndarray
        The sequence code of this :class:`Sequence`
    symbols : list
        The list of symbols, represented by the :class:`Sequence`.
    ...
    def get_alphabet(self):
        ...
        This method must be overwritten, when subclassing :class:`Sequence`.
