...
import numpy as np
from biotite.rust.sequence.phylo import neighbor_joining as rust_neighbor_joining
...
def neighbor_joining(distances: NDArray2[N, N, np.floating]) -> nx.DiGraph:
    ...
    *neighbor joining* algorithm. :footcite:`Saitou1987, Studier1988`
    ...
    ValueError
        ...
        if any matrix entry is below 0
        or if the matrix has less than 3 rows.
    ...
    parents, children, edge_distances = rust_neighbor_joining(distances)
