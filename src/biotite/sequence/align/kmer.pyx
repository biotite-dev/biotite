...
cdef class KmerIndex:
    ...
    cdef object _alph
    cdef uint8[:,:] _sim_matrix
    cdef int _k
    cdef int _n_sequences

    cdef uint64[:] _kmer_len
    cdef ptr[:] _kmer_seq
    cdef ptr[:] _kmer_pos
    ...
    def __cinit__(self, alphabet, k, similarity_rule):
        ...
        self._kmer_len = np.zeros(n_kmers, dtype=np.uint64)
    ...
    def add(self, sequence):
        ...
        cdef uint64 length
        cdef int64 kmer
        cdef uint64* kmer_ptr

        code = sequence.code
        
        if len(code) < self._k:
            raise ValueError("Sequence code is shorter than k")
        if not self._alph.extends(sequence.alphabet):
            raise ValueError(
                "The k-mer index alphabet does not extend alphabet of "
                "the sequence"
            )
        ...
        for seq_pos in range(kmers.shape[0]):
            ...
            length = self._kmer_len[kmer] + 1
            self._kmer_len[kmer] = length

            # Set the index of the sequence in the list for this kmer
            kmer_ptr = <uint64*>self._kmer_seq[kmer]
            kmer_ptr = <uint64*>realloc(kmer_ptr, length * sizeof(uint64))
            if not kmer_ptr:
                raise MemoryError()
            kmer_ptr[length-1] = self._n_sequences
            self._kmer_seq[kmer] = <ptr>kmer_ptr

            # Set the position of this kmer in the sequence
            kmer_ptr = <uint64*>self._kmer_pos[kmer]
            kmer_ptr = <uint64*>realloc(kmer_ptr, length * sizeof(uint64))
            if not kmer_ptr:
                raise MemoryError()
            kmer_ptr[length-1] = seq_pos
            self._kmer_pos[kmer] = <ptr>kmer_ptr
    

    def match(self, kmer_index):
        pass
        # TODO
    ...
    def __str__(self):
        ...
        for kmer in range(self._kmer_len.shape[0]):
            if self._kmer_len[kmer] > 0:
                ...
                line = str(tuple(symbols)) + ": "
                for i in range(self._kmer_len[kmer]):
                    line += str((
                        (<uint64*>self._kmer_seq[kmer])[i],
                        (<uint64*>self._kmer_pos[kmer])[i],
                    ))
                lines.append(line)
        return "\n".join(lines)
