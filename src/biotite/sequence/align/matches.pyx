...
def find_matches(references, query):
    ...
    #    print(code, [mapping[code][i] for i in range(mapping_lengths[code])])

    matches = [
        _find_matches[CodeType](reference.code, mapping, mapping_lengths)
...
cdef np.ndarray _find_matches(CodeType[:] reference,
