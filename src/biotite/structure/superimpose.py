...
def superimpose(fixed, mobile, atom_mask=None):
    ...
    Notes
    -----
    The `transformation` tuple can be used in :func:`superimpose_apply()` in
    order to transform another :class:`AtomArray` in the same way. This can
    ...
    structure using :func:`superimpose_apply()`.
