...
class GROFile(TextFile):
    ...
    def get_structure(self, model=None):
        ...
        model_atom_counts = np.array(
            [int(self._lines[i]) for i in model_start_i]
        )
    ...
    def set_structure(self, array):
        ...
        if isinstance(array, AtomArray):
            ...
            # Write header lines
            self._lines[0] = "Generated by biotite at {}" \
            ...
            fmt = '{:>5d}{:5s}{:>5s}{:5d}'
            for i in range(array.array_length()):
                temp_lines[i] = fmt.format(array.res_id[i], array.res_name[i],
                                           array.atom_name[i], atom_id[i])

            for i in range(array.stack_depth()):
                self._lines.append("Generated by biotite at {}, model={}"
                ...
                self._lines.append("{:>8.3f} {:>8.3f} {:>8.3f}"
